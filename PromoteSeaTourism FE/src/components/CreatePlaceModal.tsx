...
import type { CreatePlaceRequest } from "../types/place";
...
export default function CreatePlaceModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Tạo place thành công!");
      onSuccess?.();
      handleClose();
    } catch (error) {
  ...
  }, [isOpen, loadCategories, loadImages]);

  if (!isOpen) return null;

  return (
    <Modal isOpen={isOpen} onClose={handleClose} title="Tạo Place" size="xl">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    geolat: e.target.value ? parseFloat(e.target.value) : null,
                  })
                }
                disabled={loading}
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-white/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-white placeholder-white/60"
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    geolng: e.target.value ? parseFloat(e.target.value) : null,
                  })
                }
                disabled={loading}
                className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-white/50 focus:border-white/50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed text-white placeholder-white/60"
