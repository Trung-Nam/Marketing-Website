...
export default function CreateRestaurantModal({
  ...
  const [images, setImages] = useState<RestaurantImageRequest[]>([]);
  const [newImage, setNewImage] = useState<RestaurantImageRequest>({
    url: "",
    altText: "",
    caption: "",
    position: 0,
  ...
  const addImage = () => {
    ...
    const imageToAdd: RestaurantImageRequest = {
      ...newImage,
  ...
  const removeImage = (index: number) => {
    ...
    const updatedImages = newImages.map((img, i) => ({
      ...img,
      position: i,
      isCover: i === 0, // First remaining image becomes cover
    }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Tạo restaurant thành công!");
      onSuccess?.();
      onClose();
    } catch (error) {
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            {categoriesLoading ? (
              <div className="flex items-center">
                <LoadingSpinner />
                <span className="ml-2 text-white">Đang tải categories...</span>
              </div>
            ) : (
              <select
                name="categoryId"
        ...
        <div className="flex justify-end space-x-3 pt-6 border-t border-white/20">
          ...
          >
            {loading ? "Đang tạo..." : "Tạo Restaurant"}
          </button>
        </div>
      </form>
    </Modal>
