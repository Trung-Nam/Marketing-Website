...
export default function EditAccommodationModal({
  ...
  const [formData, setFormData] = useState<UpdateAccommodationRequest>({
    name: "",
    slug: "",
    summary: "",
    content: "",
  ...
  const [newImage, setNewImage] = useState<UpdateAccommodationImage>({
    url: "",
    altText: "",
    caption: "",
    position: 0,
  ...
  const loadAccommodationDetail = useCallback(async () => {
    ...
    try {
      ...
      toast.error("Không thể tải chi tiết accommodation!");
    }
  }, [accommodation]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Cập nhật accommodation thành công!");
      onSuccess?.();
      handleClose();
    } catch (error) {
  ...
  const setCoverNewImage = (index: number) => {
    setFormData({
      ...
      coverImageId: 0, // Clear coverImageId when setting new image as cover
    });
  };
  ...
  const setCoverExistingImage = (mediaId: number) => {
    setFormData({
      ...
      // Clear isCover for new images when setting existing image as cover
      addImages: formData.addImages.map((img) => ({
        ...img,
        isCover: false,
      })),
    });
  ...
  }, [isOpen, loadCategories, loadAccommodationDetail]);

  // Populate form data when accommodation detail is loaded
  useEffect(() => {
    if (accommodationDetail && accommodation) {
      setFormData({
        name: accommodationDetail.name,
        slug: accommodationDetail.slug,
        summary: accommodationDetail.summary || "",
        content: accommodationDetail.content || "",
        address: accommodationDetail.address || "",
        phone: accommodationDetail.phone || "",
        website: accommodationDetail.website || "",
        star: accommodationDetail.star || 0,
        minPrice: accommodationDetail.minPrice || 0,
        maxPrice: accommodationDetail.maxPrice || 0,
        categoryId: accommodationDetail.categoryId || 0,
        isPublished: accommodationDetail.isPublished,
        createdAt: accommodationDetail.createdAt,
        addImages: [],
        attachMediaIds: [],
        removeLinkIds: [],
        reorders: [],
        coverImageId: accommodationDetail.coverImageId || 0,
      });
    }
      ...
      <div className="max-h-[80vh] overflow-y-auto">
        <form onSubmit={handleSubmit} className="space-y-4 pr-2">
          ...
          <div>
            ...
            {/* Existing Images */}
            {accommodationDetail && accommodationDetail.images.length > 0 && (
              <div className="mb-4">
                <p className="text-sm text-white/80 mb-2">Hình ảnh hiện tại:</p>
                      ...
                      >
                        ...
                        <div className="flex gap-1">
                          {formData.coverImageId !== img.mediaId && (
                            <button
                              type="button"
                              onClick={() => setCoverExistingImage(img.mediaId)}
            ...
            <div className="space-y-2 p-3 bg-white/5 rounded-lg border border-white/10">
              <p className="text-sm text-white/80 mb-2">Thêm hình ảnh mới:</p>
              <input
                type="url"
                value={newImage.url}
                onChange={(e) =>
          ...
          <div className="mt-8 flex gap-3 justify-end">
            ...
            >
              ...
              {loading ? "Đang cập nhật..." : "Cập nhật Accommodation"}
            </button>
          </div>
        </form>
      </div>
